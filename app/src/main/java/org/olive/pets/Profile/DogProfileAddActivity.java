...
/**
 * Created by KMJ on 2017-04-23.
 */

public class DogProfileAddActivity extends Activity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        iv_DogPhoto = (ImageView) this.findViewById(R.id.dog_image_modify_a);

        // EditText를 한줄만 입력 가능하도록 설정
        et_DogName = (EditText) this.findViewById(R.id.dog_name_a);
        et_DogAge = (EditText) this.findViewById(R.id.dog_age_a);
        et_DogSex = (EditText) this.findViewById(R.id.dog_sex_a);
    ...
    public void onClick(View v) {
        id_view = v.getId();
        if(v.getId() == R.id.btn_submit_profile_a) {
            // 프로필 저장
            // 가장 뒤의 프로필의 다음 id 를 할당하자
            RealmResults<DogProfile> puppies = mRealm.where(DogProfile.class).findAllSorted("dog_id", Sort.DESCENDING);
            final int last_id = puppies.first().getDogId()+1;
            mRealm.executeTransaction(new Realm.Transaction() {
                ...
                public void execute(Realm realm) {
                    ...
                    // 여기에 posture data연결해야함
                }
            });
            mRealm.close();

            //Intent intent=new Intent(this,DogProfileListActivity.class);
            //startActivity(intent);
            this.finish();

            Toast.makeText(this, "강아지 프로필이 저장되었습니다.", Toast.LENGTH_SHORT).show();
        }else if(v.getId() == R.id.btn_UploadDogPic_a) {
